...
from common.utils.tensor_utils import fast_concat
...
class RingTensor(object):
    ...
    def __truediv__(self, other):
        if isinstance(other, RingTensor):
            ...
            raise TypeError("unsupported operand type(s) for / 'RingTensor' and ", type(other))
        return self.__class__(torch.round(new_value * self.scale), self.dtype, self.device)
    ...
    def __floordiv__(self, other):
        if isinstance(other, RingTensor):
            ...
            raise TypeError("unsupported operand type(s) for // 'RingTensor' and ", type(other))
        return self.__class__(new_value * self.scale, self.dtype, self.device)
    ...
    def clone(self):
        return self.__class__(self.tensor.clone(), self.dtype, self.device)
    ...
    def arange(self, start, end, step=1):
        """
        获取一个从start到end的RingTensor
        :return:
        """
        return self.__class__(torch.arange(start, end, step, dtype=data_type, device=self.device), self.dtype)
    ...
    def flatten(self):
        new = self.__class__(self.tensor.flatten(), self.dtype)
        new.bit_len = self.bit_len
        return new
