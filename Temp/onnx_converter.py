...
from application.NN.model import model_of_layers
...
def from_pytorch(pytorch_model, dummy_input, party, device):
    ...
    :param pytorch_model: 已经训练好的pytorch_model
...
def _to_secure_model(onnx_model, party, device):
    ...
    secure_model = model_of_layers.ModelOfLayers(str(input_names), str(output_names[0]))
    ...
    if party.party_id == 1:
        for node in onnx_model.graph.node:
            ...
            if node.op_type == "Identity":
                ...
                party.send_tesnor(value)
                secure_model.output[node.output[0]] = ArithmeticSecretSharing(
                    RingTensor(value, DTYPE, SCALE), party)
                ...
                w0, w1 = ArithmeticSecretSharing.share(
                    RingTensor.convert_to_ring(parameters[node.input[1]]), 2)
                party.send_ring_tensor(w0)

                if len(node.input) == 2:
                    b0 = b1 = None
                else:
                    if node.input[2] in parameters:
                        value = RingTensor.convert_to_ring(parameters[node.input[2]])

                        b0, b1 = ArithmeticSecretSharing.share(value, 2)
                        party.send_ring_tensor(b0)
                    else:

                        b0, b1 = ArithmeticSecretSharing.share(
                            RingTensor.convert_to_ring(secure_model.output[node.input[2]].value), 2)
                        party.send_ring_tensor(b0)
                ...
                if node.input[1] in parameters:
                    ...
                    w0, w1 = ArithmeticSecretSharing.share(
                        RingTensor.convert_to_ring(secure_model.output[node.input[1]].value), 2)
                if node.input[2] in parameters:
                    b0, b1 = ArithmeticSecretSharing.share(
                        RingTensor.convert_to_ring(parameters[node.input[2]]), 2)
                else:
                    b0, b1 = ArithmeticSecretSharing.share(
                        RingTensor.convert_to_ring(secure_model.output[node.input[2]].value), 2)
                party.send_ring_tensor(w0)
                ...
                if node.input[0].find("bias") != -1:
                    ...
                    b0, b1 = ArithmeticSecretSharing.share(RingTensor(bias), num_of_party=2)
                    party.send_torch_array(b0)

                    secure_model.output[node.input[0]] = ArithmeticSecretSharing(b1, party, device)
                ...
                w0, w1 = ArithmeticSecretSharing.share(RingTensor(weight), num_of_party=2)
                party.send_torch_array(w0)

                layer = layers.SecTranspose(ArithmeticSecretSharing(w1, party, device), device)
                layer.set_input_and_output(node.input[0], node.output[0])
                secure_model.add(layer)

            elif node.op_type == "Constant":  # 这个节点的value 是为了Reshape节点的形状，只有输出，没有输入
                output_name = node.output[0]
                secure_model.output[output_name] = ArithmeticSecretSharing(RingTensor.convert_to_ring(attributes["value"]), party, device)
    ...
    if party.party_id == 0:
        for node in onnx_model.graph.node:
            ...
            if node.op_type == "Identity":
                value = party.receive_tensor()
                value = RingTensor.convert_to_ring(value)
                secure_model.output[node.output[0]] = ArithmeticSecretSharing(value, party)

            if node.op_type == "Conv":
                pads = attributes["pads"]
                strides = attributes["strides"]

                w0 = party.receive_ring_tensor()
                if len(node.input) == 2:
                    b0 = None
                else:
                    b0 = party.receive_ring_tensor()
                ...
                if node.input[0].find("bias") != -1:
                    ...
                    secure_model.output[node.input[0]] = ArithmeticSecretSharing(b0, party, device)
                ...
                layer = layers.SecTranspose(ArithmeticSecretSharing(w0, party, device))
                layer.set_input_and_output(node.input[0], node.output[0])
                secure_model.add(layer)

            elif node.op_type == "Constant":  # 这个节点的value 是为了Reshape节点的形状，只有输出，没有输入
                secure_model.output[node.output[0]] = ArithmeticSecretSharing(
                    RingTensor.convert_to_ring(attributes["value"]), party, device)
...
def _get_input_output_names(onnx_model):
    ...
    """
    input_names = [inputs.name for inputs in onnx_model.graph.input]
